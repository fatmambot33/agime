...
SCRIPTS := build.sh configure.sh sync.sh backup.sh update.sh image.sh add_tool.sh restore.sh scripts/build_lib.sh scripts/build_steps.sh scripts/build_custom_image.sh scripts/optional_tools/common.sh scripts/optional_tools/github.sh scripts/optional_tools/himalaya.sh scripts/optional_tools/coding_agent.sh scripts/run_security_audit.sh scripts/install_security_audit_cron.sh tests/smoke_dry_run.sh tests/idempotency_dry_run.sh tests/security_template_checks.sh tests/sync_hermetic.sh tests/security_audit_scripts_hermetic.sh tests/backup_restore_hermetic.sh tests/configure_backup_hermetic.sh tests/configure_autoload_env_hermetic.sh tests/configure_image_interactive_hermetic.sh tests/ownership_config_dir_hermetic.sh tests/post_install_helpers_hermetic.sh

.PHONY: check check-strict ensure-tools syntax lint fmt-check smoke idempotency security sync-test backup-restore-test interactive-backup-test interactive-env-autoload-test interactive-image-test ownership-config-test post-install-helpers-test security-audit-scripts security-audit install-security-cron

check: syntax smoke idempotency security sync-test backup-restore-test interactive-backup-test interactive-env-autoload-test interactive-image-test ownership-config-test post-install-helpers-test security-audit-scripts

check-strict: syntax lint fmt-check smoke idempotency security sync-test backup-restore-test interactive-backup-test interactive-env-autoload-test interactive-image-test ownership-config-test post-install-helpers-test security-audit-scripts
