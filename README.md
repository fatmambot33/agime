...
- `scripts/run_security_audit.sh`: writes OpenClaw security audit output to timestamped logs.
- `scripts/install_security_audit_cron.sh`: installs a cron entry to run the security audit script.
- `Makefile`: local quality checks (`make check`, `make check-strict`, `make lint`, `make fmt-check`, `make smoke`).
