...
ask_var() {
  ...
  case "$var_name" in
    ...
    OPENCLAW_CODING_AGENT_REQUIRE_VERSION_CHECK) OPENCLAW_CODING_AGENT_REQUIRE_VERSION_CHECK=$value ;;
    *)
...
OPENCLAW_CODING_AGENT_REQUIRE_VERSION_CHECK=$OPENCLAW_CODING_AGENT_REQUIRE_VERSION_CHECK
EOF2
fi
...
if [ "${OPENCLAW_ENABLE_GITHUB_SKILL:-0}" = "1" ]; then
  export OPENCLAW_GH_CLI_PATH OPENCLAW_GH_REQUIRE_AUTH
fi
...
if [ "${OPENCLAW_ENABLE_CODING_AGENT_SKILL:-0}" = "1" ]; then
  export OPENCLAW_CODING_AGENT_BACKEND OPENCLAW_CODING_AGENT_REQUIRE_VERSION_CHECK
fi
