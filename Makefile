...
SCRIPTS := build.sh sync.sh setup.sh backup.sh restore.sh update.sh scripts/build_lib.sh scripts/build_steps.sh scripts/lib/common.sh scripts/lib/sync.sh scripts/lib/setup.sh scripts/lib/update.sh tests/smoke_dry_run.sh tests/idempotency_dry_run.sh tests/sync_hermetic.sh tests/backup_restore_hermetic.sh tests/failure_paths_hermetic.sh tests/security_template_checks.sh tests/build_public_preflight_hermetic.sh tests/sync_env_edge_cases_hermetic.sh tests/template_render_hermetic.sh tests/sync_compat_shims_hermetic.sh

.PHONY: check check-strict ensure-tools syntax lint fmt-check smoke idempotency sync-test backup-restore-test failure-paths security public-preflight sync-env-edges template-render sync-compat-shims

check: syntax smoke idempotency sync-test backup-restore-test failure-paths security public-preflight sync-env-edges template-render sync-compat-shims

check-strict: syntax lint fmt-check smoke idempotency sync-test backup-restore-test failure-paths security public-preflight sync-env-edges template-render sync-compat-shims
