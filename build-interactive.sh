...
ask_var() {
  ...
  case "$var_name" in
    ...
    OPENCLAW_GH_AUTO_INSTALL) OPENCLAW_GH_AUTO_INSTALL=$value ;;
    OPENCLAW_GH_REQUIRE_AUTH) OPENCLAW_GH_REQUIRE_AUTH=$value ;;
    ...
    OPENCLAW_HIMALAYA_AUTO_INSTALL) OPENCLAW_HIMALAYA_AUTO_INSTALL=$value ;;
...
if [ "${OPENCLAW_ENABLE_GITHUB_SKILL:-0}" = "1" ]; then
  ...
  ask_var OPENCLAW_GH_AUTO_INSTALL "Auto-install gh if missing (1=yes, 0=no)" "1"
...
if [ "${OPENCLAW_ENABLE_HIMALAYA_SKILL:-0}" = "1" ]; then
  ...
  ask_var OPENCLAW_HIMALAYA_AUTO_INSTALL "Auto-install himalaya if missing (1=yes, 0=no)" "1"
...
OPENCLAW_GH_AUTO_INSTALL=$OPENCLAW_GH_AUTO_INSTALL
OPENCLAW_GH_REQUIRE_AUTH=$OPENCLAW_GH_REQUIRE_AUTH
EOF2
fi
...
OPENCLAW_HIMALAYA_AUTO_INSTALL=$OPENCLAW_HIMALAYA_AUTO_INSTALL
...
if [ "${OPENCLAW_ENABLE_GITHUB_SKILL:-0}" = "1" ]; then
  export OPENCLAW_GH_CLI_PATH OPENCLAW_GH_AUTO_INSTALL OPENCLAW_GH_REQUIRE_AUTH
fi

if [ "${OPENCLAW_ENABLE_HIMALAYA_SKILL:-0}" = "1" ]; then
  export OPENCLAW_HIMALAYA_CLI_PATH OPENCLAW_HIMALAYA_AUTO_INSTALL OPENCLAW_HIMALAYA_REQUIRE_CONFIG OPENCLAW_HIMALAYA_CONFIG_PATH
fi
